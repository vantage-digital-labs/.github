# Vantage Digital Labs

Enterprise AI infrastructure for game studios.

We build production-grade tools that give game developers access to cognitive NPC systems, real-time voice synthesis, and behavioral analytics — without having to manage ML infrastructure.

---

### Our SDKs

| Repository | Description | Version |
|------------|-------------|---------|
| [core-sdk](https://github.com/vantage-digital-labs/core-sdk) | Node.js / Browser SDK | ![npm](https://img.shields.io/npm/v/@vantage-labs/core-sdk) |
| [unity-sdk](https://github.com/vantage-digital-labs/unity-sdk) | Unity 2021.3+ Package | ![version](https://img.shields.io/badge/version-2.4.0-blue) |
| [unreal-plugin](https://github.com/vantage-digital-labs/unreal-plugin) | Unreal Engine 5.2+ Plugin | ![version](https://img.shields.io/badge/version-2.4.0-blue) |

---

### Platform

- **Inference**: GCP Vertex AI with Gemini model routing
- **Compute**: Alibaba Cloud ECS (APAC), GCP (Global)
- **Voice**: Real-time STT→LLM→TTS pipeline
- **Languages**: Japanese · Korean · Chinese · English
- **Scale**: 500M+ API calls/month · 150K concurrent connections

---

### Links

- Website: [vantage-digital.online](https://vantage-digital.online)
- Documentation: [vantage-digital.online/resources/documentation](https://vantage-digital.online/resources/documentation)
- API Reference: [vantage-digital.online/resources/api-reference](https://vantage-digital.online/resources/api-reference)
- Discord Community: [discord.gg/fNB4UB93](https://discord.gg/fNB4UB93)
- Contact: store@vantage-digital.online
